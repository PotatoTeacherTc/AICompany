...
| API contracts and application | `api/contracts.py`, `api/task_api.py`, `api/app.py`, `api/errors.py` | Transport-neutral request/response DTOs define task creation plus single/list retrieval. The FastAPI app factory injects application services, exposes `/health`, and returns a uniform sanitized global error response. TaskApi depends only on AutomationService and TaskQueryService, so future workspace and authentication adapters can be added without importing core Pipelines or repositories. Task HTTP routes and authentication are not implemented yet. |
