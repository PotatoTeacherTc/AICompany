...
| API contracts and application | `api/contracts.py`, `api/task_api.py`, `api/app.py`, `api/errors.py` | Transport-neutral request/response DTOs define task creation plus single/list retrieval. The FastAPI app factory injects application services, exposes `/health`, `POST /tasks`, `GET /tasks/{task_id}`, `GET /tasks`, `POST /tasks/{task_id}/cancel`, and `POST /tasks/{task_id}/retry`, and returns uniform sanitized error responses. Invalid control states return 409 without exception details. TaskApi depends only on AutomationService and TaskQueryService, so future workspace and authentication adapters can be added without importing core Pipelines or repositories. Authentication is not implemented yet. |
