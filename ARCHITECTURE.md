...
| Collaboration mission contract | `core/mission.py` | Mission is an immutable, serializable unit-of-work contract containing a unique ID, title, objective, requester identity, explicit workspace ownership, timezone-aware creation time, and isolated extension metadata. Required fields are validated with value-free errors. Workspace ownership prevents future collaboration records from becoming tenant-ambiguous. Lifecycle state and locking are deliberately excluded until Mission 71, and this contract does not execute work. |
