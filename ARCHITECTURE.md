...
| API contracts and application | `api/contracts.py`, `api/task_api.py`, `api/app.py`, `api/errors.py` | Transport-neutral request/response DTOs define task creation plus single/list retrieval. The FastAPI app factory injects application services, exposes `/health`, `POST /tasks`, `GET /tasks/{task_id}`, and `GET /tasks`, and returns uniform sanitized error responses. TaskApi depends only on AutomationService and TaskQueryService, so future workspace and authentication adapters can be added without importing core Pipelines or repositories. Task control routes and authentication are not implemented yet. |
