...
|---|---|---|
| Task model | `core/task.py`, `core/status.py`, `core/result.py` | Task owns ID, text, optional structured parameters, an optional parent Task ID, lifecycle timestamps, result, task type, pipeline, and bounded retry metadata. It exposes PENDING/QUEUED/RUNNING/SUCCESS/FAILED/SKIPPED transitions. PipelineResult serializes the common result fields. |
| Queue and execution | `core/task_queue.py`, `core/worker.py` | FIFO queue accepts optional ExecutionHistory and retry-limit dependencies. Queue and Worker record QUEUED, RUNNING, retry, and terminal updates through the same task ID; History upserts the record so result, timestamps, duration, retry count, and sanitized error type remain synchronized without duplicates. |
