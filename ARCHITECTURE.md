...
| Artifact boundary | `core/artifact_manager.py`, `core/artifact_repository.py` | ArtifactManager creates stable artifact IDs and registers file metadata through an injected InMemory or file-backed repository. Standard metadata includes ID, type, MIME type, filename, size, creation time, producer pipeline, and a reserved workspace ID. PipelineResult carries those records into ExecutionHistory without changing its existing fields. |
