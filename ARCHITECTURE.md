...
| API contracts | `api/contracts.py`, `api/task_api.py` | Transport-neutral request/response DTOs define task creation plus single/list retrieval. TaskApi depends only on AutomationService and TaskQueryService, so a future HTTP server, workspace boundary, and authentication adapter can be added without importing core Pipelines or repositories. No web server or authentication is implemented. |
