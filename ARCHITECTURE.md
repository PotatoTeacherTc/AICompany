...
| AI provider boundary | `providers/base.py`, `providers/models.py`, `providers/mock_provider.py`, `providers/factory.py` | Provider-neutral request/response and usage metadata. MockProvider is the offline default; CONTENT and RESEARCH can receive an injected provider and persist only provider/model/token/cost metadata through PipelineResult and ExecutionHistory, never API keys. |
