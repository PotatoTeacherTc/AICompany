...
| Worker context | `core/worker_context.py` | ContextBuilder converts only a validated Mission into a WorkerContext containing Mission and Workspace identity plus the normalized objective. Metadata is copied through a scalar-only filter that removes credential, token, password, and raw-prompt keys. It does not read files, create worktrees, call providers, or execute work. |
